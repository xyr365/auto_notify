...
class AutoSign:
    ...
    def getMOD(self):
        ...
        res = self.session.post(url, headers=headers, data=json.dumps({}), verify=False, allow_redirects=False)
        i = 0
        while str(res.headers.get('Set-Cookie')).rfind('MOD_AUTH_CAS') == -1 :
            res = self.session.get(res.headers["location"], headers=headers, verify=False, allow_redirects=False)
            i += 1
            if i > 6 :
                print("重定向次数异常")
                exit()
